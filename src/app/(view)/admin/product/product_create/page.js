...
const CreateProduct = () => {
    ...
    const [fields, setFields] = useState([{ product_name: '', status_id: '', file_path: '', product_description: '', brand_id: '', model_id: '', category_id: '', sub_category_id: '', period_id: '', unit_id: '', warranty_id: '', material_id: '', color_id: '', type_id: '', product_price: '', product_weight: '', created_by: created, }]);
    ...
    const handleAddMore = () => {
        ...
        if (!isNaN(numToAddInt) && numToAddInt > 0) {
            ...
            for (let i = 0; i < numToAddInt; i++) {
                newInputValues.push({
                    product_name: '', status_id: '', file_path: '', product_description: '', brand_id: '', model_id: '', category_id: '', sub_category_id: '', period_id: '', unit_id: '', warranty_id: '', material_id: '', color_id: '', type_id: '', product_price: '', product_weight: '', created_by: created,
                });
            }
    ...
    const handleSubmit = (event) => {
        ...
        //     // const file_s = form.file_s.value || form?.file_s[index]?.value
        //     // const file_m = form.file_m.value || form?.file_m[index]?.value
        //     // const file_ms = form.file_ms.value || form?.file_ms[index]?.value
        //     const file_s = (form.file_s && form.file_s?.value) || (form?.file_s && form.file_s[index]?.value) || '';
        //     const file_m = (form.file_m && form.file_m?.value) || (form?.file_m && form.file_m[index]?.value) || '';
        //     const file_ms = (form.file_ms && form.file_ms?.value) || (form?.file_ms && form.file_ms[index]?.value) || '';

        // Add your form submission logic here using the 'fields' state.
        // if (!file_path.trim() || !file_s.trim() || !file_m.trim() || !file_ms.trim()) {
        //     // Show error message
        //     setFilePathError("File Path are required fields.");
        //     return; // Exit function without submitting the form
        // }
        // else{
        //     setFilePathError('')
        // }
        // if (!(form.file_path?.value || form.file_path[index]?.value) || !(form.file_ms?.value || form.file_ms[index]?.value) || !(form.file_s?.value || form.file_s[index]?.value) || !(form.file_m?.value || form.file_m[index]?.value))
        // if (!file_path.trim() || !file_s.trim() || !file_m.trim() || !file_ms.trim()) {
        //     // Show error message
        //     setFilePathError("File Path are required fields.");
        //     return; // Exit function without submitting the form
        // }
        // if (!form?.file_path[index]?.value || !form?.file_m[index]?.value || !form?.file_s[index]?.value || !form.file_ms[index]?.value) {
        //     setFilePathError(['Please fill in all file fields.']);
        //     return;
        // }
        // else {
        //     setFilePathError('')
